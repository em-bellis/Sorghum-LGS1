...
coords <- read.table('/Users/emilywork/Desktop/LGS1/MSfigs/maps_gwas/hmp_ne.lat.lon.txt', header=F)


data(worldMapEnv)

##
pdf('/Users/emilywork/Desktop/LGS1/LGS1_revision_8.19/Fig1.pdf')
map(database="world", xlim=c(-20,60),ylim=c(-40,45),axes=TRUE)
plot(all, add=T, legend=F, col=viridis(n=100)) #joyce votes for viridis
map(database="world", xlim=c(-20,60),ylim=c(-40,45),add=T, col="white", lwd=0.75)
...
map(database="world", xlim=c(-20,60),ylim=c(-40,45), lwd=0.5)
plot(all_within, add=T, legend=F, col=viridis(n=100)) #joyce votes for viridis
map(database="world", xlim=c(-20,60),ylim=c(-40,45),add=T, col="black", lwd=0.5)
...
###part b
striga_score <- read.table('/Users/emilywork/Desktop/LGS1/LGS1_revision_8.19/global_200.fam', header=F)
bap <- read.table('/Users/emilywork/Desktop/LGS1/referenced_bap.txt', header=T)
...
par(fig = c(0,1,0,1))
map(database="world", xlim=c(-20,100),ylim=c(-40,45), col="grey70", lwd=0.5)
